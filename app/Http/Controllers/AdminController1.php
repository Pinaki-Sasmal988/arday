...
/******************all results begin**************************/    
    public function allColleges(){
        ...
        $college_categories=$this->query_resolver_get($arg.'\College');
...
if(($request->is_popular==null)||($request->is_popular=='')){
    ...
    if($request->college_banner==null){
        ...
        $bannerimageName = time().'.'.$request->college_banner->extension();  
    $request->college_logo->move(public_path('assets/img/college_banner'), $bannerimageName);
    }
    if($request->brochure){
    $brochureName = time().'bro.'.$request->brochure->extension();  
    $request->brochure->move(public_path('assets/brochure'), $brochureName);
...
public function searchContent(Request $request){
    ...
    $tb='App\Models';
    $college_categories=$this->query_resolver_get($tb.'\College');
