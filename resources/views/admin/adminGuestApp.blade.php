...
<div class="card">
        ...
        <div class="table-responsive">
            ...
            <div class="m-3">
                <div class="row">
                    ...
                    <div class="col-md-8">
                       <form method="POST" action="{{route('searchContent')}}" name="guestapp-search-form">
                       @csrf 
                        ...
                        <input type="text" name="filter_type" class="d-none" value="guestapp-search-filter">
                    </form>
                    </div>
                </div>
            </div>
            ...
            <table class="table mb-0 thead-border-top-0 table-striped">
                ...
                <tbody class="list" id="products">
                    @if(count($guest_applications)>0)
                    @foreach($guest_applications as $app)
                    @php
        ...
        <div class="card-body text-right">
            @if(count($guest_applications)<20)
            {{count($guest_applications)}} @else{20} @endif<span class="text-muted">of {{count($guest_applications)}}</span> <a href="#"
               class="text-muted-light"><i class="material-icons ml-1">arrow_forward</i></a>
        </div>
        <div class="text-center mx-auto">
            {{ $guest_applications->onEachSide(5)->links() }}
        </div>
